...
回测页仅提供 vn.py 股票池组合分钟回测。策略目录包含完整开盘突破策略以及条件 1、2、3 专用策略，单次接受 1–1000 只 A 股代码。

**真实约束**：A 股 T+1、主板/创业板/科创板/北交所交易规则、整手和科创板最低买入量、历史涨跌停、手续费、印花税、滑点、最大持仓、3% 现金预留、可选单分钟成交量 10% 限制和下一分钟开盘撮合。

**组合管理**：动态等权或评分加权；等权额度按当前可用资金和剩余持仓槽位实时刷新，同分钟候选按条件优先级和固定规则确定顺序。

输出净值与回撤、每日账本、闭合交易、逐股统计、信号诊断、拒单原因和期末持仓。SSE 支持进度、切页/刷新重连和主动取消。

信号价格默认使用本地 `adj_factor_tushare` 前复权，撮合仍使用原始分钟价格；也可显式选择 `raw`。当前回测入口面向 A 股股票池，不提供旧单标的、因子回测、参数优化或 Walk-forward 页面。
...
个股分析入口现已替换为 FinSight 深度研报工作台；旧后端接口暂时保留，供已有调用方兼容迁移。

- **分阶段任务**：按需选择资料采集与分析任务，先展示预计执行范围，再创建运行。
- **异步运行管理**：可查看阶段、进度、消息与日志，支持取消、删除已结束任务；默认仅允许一份研报同时运行。
- **多格式交付**：成功后下载 Markdown、DOCX、PDF 与日志；DOCX/PDF 由锁定模板和 Microsoft Word 转换。
- **运行前健康检查**：检查 FinSight runtime、独立 Python、模板与配置是否就绪；接口不会暴露本机绝对路径。
- **安全边界**：股票代码和名称经过严格校验，输出目录受控，真实 API Key 只保存在各机器的 `.env` 中。

该能力只支持 Windows x64 + Microsoft Word。安装、环境锁定和跨机器 PDF 版式验收见 [deep-report-windows.md](./deep-report-windows.md)。
