"""Condition-2-only variant of the opening-breakout strategy."""
...
class OpeningBreakoutCondition2Strategy(OpeningBreakoutPoolStrategy):
    """Keep only the local condition 2 for entries; exits stay unchanged."""
    ...
    def _buy_diagnostic(self, symbol: str, bar: BarData, context: PortfolioContext) -> dict[str, object] | None:
        ...
        if not diagnostic or "condition_2" not in diagnostic.get("matched_condition_ids", []):
            return None
        return {
            **diagnostic,
            "matched_conditions": [self._condition_label("condition_2")],
            "matched_condition_ids": ["condition_2"],
            "primary_reason": "condition_2",
        }
